...
static stdx::atomic<int> atomic_int;
static stdx::shared_ptr<int> shared_ptr;
...
void test_select()
{
    typedef cppdatalib::json::stream_writer json_writer;
...
int readme_simple_test()
{
    ...
    try {
        ...
        core::convert(p, my_value);         // Read in to core::value from STDIN as JSON
        core::convert(w, static_cast<const core::value &>(my_value));         // Write core::value out to STDOUT as JSON
...
int readme_simple_test2()
{
    ...
    try {
        core::convert(json::parser(std::cin), json::stream_writer(std::cout));        // Write core::value out to STDOUT as JSON
    } catch (const core::error &e) {
        std::cerr << e.what() << std::endl; // Catch any errors that might have occured (syntax or logical)
    }
...
int readme_simple_test3()
{
    ...
    try {
        core::convert(json::stream_writer(std::cout), from_json(std::cin));        // Write core::value out to STDOUT as JSON
    } catch (const core::error &e) {
        std::cerr << e.what() << std::endl; // Catch any errors that might have occured (syntax or logical)
    }
...
int readme_simple_test4()
{
    ...
    try {
        ...
        while (p.busy());
        p.end();
        core::convert(core::convert(p, my_value), m2);
...
void test_attributes()
{
    try
    {
        ...
        cppdatalib::core::convert(std::cout, from_bson(bstream));
...
#if 0
    TestRange("float_from_ieee_754", std::numeric_limits<uint32_t>::max(), cppdatalib::core::float_cast_from_ieee_754,
              cppdatalib::core::float_from_ieee_754, true, [](const auto &f, const auto &s){return f != s && !isnan(f) && !isnan(s);});
    TestRange("float_to_ieee_754", std::numeric_limits<uint32_t>::max(), [](const auto &f){return f;},
...
{
    stdx::shared_ptr<int> shared_ptr = stdx::shared_ptr<int>(new int(0));

    cppdatalib::cleanup cleanup;
    (void) cleanup;

    cppdatalib::hex::encode_accumulator outa(std::cout);
    cppdatalib::core::dump::stream_writer writer(cppdatalib::core::ostream_handle(outa), 2);

    //cppdatalib::core::convert(writer, cppdatalib::core::value(cppdatalib::core::object_t()));

    cppdatalib::crc32::accumulator crc(&outa);

    crc.accumulate("12345678");
    crc.end();
    std::cout << std::endl;
...
#if 0
    cppdatalib::core::value mval;
    cppdatalib::core::value_builder builder(mval);
    std::string temp;
    //cppdatalib::core::ostring_wrapper_stream wrapper(temp);
    cppdatalib::hex::encode_accumulator accumulator(builder);
    cppdatalib::core::identity_accumulator iaccum(&accumulator);
    cppdatalib::hex::decode_accumulator deaccumulator(std::cout);

    iaccum.accumulate("My message, whatever it is :)");
    iaccum.end();

    std::cout << mval << std::endl;

    deaccumulator.accumulate(mval.as_string());
    deaccumulator.end();

    cppdatalib::core::icstring_wrapper_stream cwrapper("My message, whatever it is :)");
    cppdatalib::base64::encode_accumulator n(cwrapper);
    for (size_t i = 0; i < 10; ++i)
        std::cout << (char) n.get();
    std::cout << std::endl;
    ...
    cppdatalib::core::value(std::chrono::system_clock::now()).cast(sec_systemclock);
    ...
    cmp = cppdatalib::core::value(std::complex<float>(4.55f, -7)).as<decltype(cmp)>();
    cppdatalib::core::value(std::complex<float>(4.55f, -7)).cast(cmp).cast(vec);
    std::cout << cmp << std::endl;
    std::cout << vec << std::endl;

    std::atomic<float> aflt;

    aflt = 0.0;
    cppdatalib::core::value scratch = aflt;
...
#if 0
    std::u16string u16s, u16s2;
    u16s.push_back(L'π');
    u16s.push_back(' ');
    u16s.push_back(L'φ');
    std::cout << cppdatalib::core::value(u16s) << std::endl;

    cppdatalib::core::value(u16s).cast(u16s2);
    std::cout << cppdatalib::core::value(u16s2) << std::endl;
#endif

    std::cout << cppdatalib::core::value(cppdatalib::core::value("My String").as<std::wstring>()) << std::endl;
#endif
    ...
    stdx::unique_ptr<std::ifstream> infile;
    stdx::unique_ptr<std::ofstream> outfile;
    stdx::unique_ptr<cppdatalib::core::stream_input> input;
    stdx::unique_ptr<cppdatalib::core::stream_handler> output(new cppdatalib::core::stream_handler());
    ...
    out = argv[2];
#else
    in = "F:\\Test_Data\\medium-dict.json";
    out = "internal";
#endif
    ...
    else if (in_scheme.empty() || in_scheme == "file")
    {
        ...
        infile.get()->open(in.c_str(), std::ios::binary | std::ios::in);
        if (!*infile.get())
        {
    ...
    else if ((out_scheme.empty() || out_scheme == "file") && out != "null")
    {
        ...
        outfile.get()->open(out.c_str(), std::ios::binary | std::ios::out | std::ios::trunc);
    ...
    try
    {
        ...
        cppdatalib::core::convert(*input.get(), buffer);
